...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def connect_code_list_view(self):
        ...
        self.code_list = self.objCodeMgr.get_code_list(1) + self.objCodeMgr.get_code_list(2)
        self.name_list = list(map(self.objCodeMgr.get_code_name, self.code_list))
        ...
        self.df_code_name_latest_db = pd.merge(self.df_code_name_from_server, df_latest_min_db, how='outer', on='종목코드')
        self.df_code_name_latest_db = pd.merge(self.df_code_name_latest_db, df_latest_day_db, how='outer',on='종목코드')
        self.df_code_name_latest_db["파일명"] = "분봉_" + self.df_code_name_latest_db["종목코드"] + "_" +self.df_code_name_latest_db["종목명"] + ".db"
    ...
    def get_df_latest_min_db(self):
        ...
        for file in self.file_lists :
            ...
            code = file.split("_")[1]
            try : 
                cursor.execute("SELECT date FROM stock ORDER BY date DESC LIMIT 1")
                from_date = cursor.fetchall()[0][0]
                self.dict_file_latest_list[code] = from_date
            except :
                print("Exception : " + file + " has no from_date. Set from_date to 0")
    ...
    def get_df_latest_day_db(self):
        ...
        try : 
            for db_code in db_code_list:
                cmd = "SELECT date FROM {} ORDER BY date DESC LIMIT 1".format(db_code[0])
                cursor.execute(cmd)
    ...
    def update_price_db_min(self, tick_unit = '분봉'):
        ...
        for index, row in tqdm.tqdm(self.df_code_name_latest_db.iterrows(), total=self.df_code_name_latest_db.shape[0]): 
            ...
            with sqlite3.connect(file_path) as con:
                ...
                df = df.iloc[::-1]
                df.to_sql(name = "stock", con = con, if_exists='append', index_label='date')
